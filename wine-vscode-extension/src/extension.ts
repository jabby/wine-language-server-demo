...
export function activate(context: ExtensionContext) {
	...
	languages.setLanguageConfiguration('wd', {
	...
	let serverOptions: Executable = {
		...
		args: [ "-jar", "/home/jabberwock/git/github/wine-language-server-demo/wine-language-server/target/wine-language-server-0.0.1-SNAPSHOT-jar-with-dependencies.jar" ]
	};

	// Options to control the language client
	let clientOptions: LanguageClientOptions = {
		// Register the server for wd
		documentSelector: ['wd'],
		synchronize: {
			configurationSection: 'wd',
			// Notify the server about file changes to .wd files contain in the workspace
			fileEvents: [
				workspace.createFileSystemWatcher('**/*.wd'),
			],
		}
	};
	...
	let languageClient = new LanguageClient('wd','Wine Descriptor Language Support', serverOptions, clientOptions);
	...
	workspace.registerTextDocumentContentProvider('wd', provider);

	item.text = 'Starting Wine Descriptor Language Server...';
	toggleItem(window.activeTextEditor, item);
...
function getServerConfiguration():WorkspaceConfiguration {
	return workspace.getConfiguration('wd');
}